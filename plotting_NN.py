...
from src.model_factory import fit_and_score, model_MLP
...
MLP_score = fit_and_score(mlp_model, X_train, y_train, X_test, y_test)
...
_vmax = max(
    _cm(y_test, mlp_model.predict(X_test)).max(),
    1
)
plot_confusion_matrix(mlp_model, X_test, y_test, 'MLP Classifier', vmax=_vmax)
...
plot_shap_summary(mlp_model, X_train, 'MLP Classifier', log_scale=False, pretty_names=ds_pretty_names)
#plot_shap_summary(lr_model, X_train, 'Logistic Regression', log_scale=False, pretty_names=ds_pretty_names)
misclassified_dt_idx = int((mlp_model.predict(X_test) != y_test.values).argmax())
plot_lime_explanation(mlp_model, X_train, X_test, 'MLP Classifier', instance_idx=misclassified_dt_idx)
...
# %%
plot_shap_summary(mlp_model, X_train, 'MLP Classifier', dir_path = "src/Director/", features=6, pretty_names = {
    "cat__marital-status_ Married-civ-spouse": "Married",
...
#plot_fairness(lr_model, X_test, y_test, X_test_original = X_test, category = "sex", model_name = "Logistic
# Regression", dir_path = \
#    "src/Director/")
#
...
#              dir_path = "src/Director/")
#
#plot_fairness(lr_model, X_test, y_test, X_test_original = X_test, category = "race", model_name = "Logistic
# Regression",
#              dir_path = "src/Director/")
#
#plot_fairness(dt_model, X_test, y_test, X_test_original = X_test, category = "sex", model_name = "Decision Tree",
...
plot_lime_explanation(mlp_model, X_train, X_test, 'MLP Classifier', instance_idx=rejected_idx, num_features=6, dir_path=
...
#plot_lime_explanation(lr_model, X_train, X_test, 'Logistic Regression', instance_idx=rejected_idx, num_features=7,
# dir_path =
#"src/EndUser/", features_hidden = ['age', 'sex', 'race', 'native-country', 'fnlwgt',
...
#plot_roc_curve({"Decision Tree": dt_model, "Logistic Regression": lr_model}, X_test, y_test, dir_path =
# "src/DataScientist/")

#plot_feature_distribution(X_train=X_train, categorical=categorical_cols, numerical=numeric_cols,
# dir_path="src/DataScientist/",
#                          numerical_pretty={
#                              'age': 'Age',
...
plot_bias_report(mlp_model, X_test, y_test, X_test, category='sex', model_name='MLP Classifier')
#plot_bias_report(lr_model, X_test, y_test, X_test, category='sex', model_name='Logistic Regression')
plot_bias_report(mlp_model, X_test, y_test, X_test, category='race', model_name='MLP Classifier')
#plot_bias_report(lr_model, X_test, y_test, X_test, category='race', model_name='Logistic Regression')

#plot_business_error_summary({"Decision Tree": dt_model, "Logistic Regression": lr_model}, X_test, y_test,
# dir_path = "src/Director/")
